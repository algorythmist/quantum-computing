...
class QCProblemsTestCase(unittest.TestCase):
    ...
    def test_problem_6_1_3(self):
        Uf1 = build_deutsch_oracle(constant0)
        self.assertTrue(is_unit_matrix(Uf1))

        Uf2 = build_deutsch_oracle(constant1)
        print(Uf2)

        Uf3 = build_deutsch_oracle(balanced1)
        self.assertTrue(np.array_equal(Uf3, CNOT))

        Uf4 = build_deutsch_oracle(balanced2)
        print(Uf4)
